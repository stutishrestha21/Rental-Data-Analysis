...
"
  ...
  output$map_comment <- renderText({
    'This is the Map of the business that is in the data.'
  })
  
  output$busn_type_comment <- renderText({
    'We have different business types in our data. So we sorted out business type 
    and we counted how many businesses there are in each type. The highest business
    type is 4701- Hotels(EXC Casino Hotels) and motel. They have total of around
    235.  The lowest is the 2060 Casino Hotels and 2302 Theater Co and Dinner Theater 
    with less than 10 in total.
    '
  })
  
  output$start_date_comment <- renderText({
    'We have a graph here that shows business types and when they were started.
    In x axis we have business start date from before 1980 and till 202 in twenty
    a year apart. As we know from "Count by Business Type" that hotel motels types are
    ones with the highest count. In this graph we can see that Hotels and motels were the
    ones that were started before anyone else and till 2020 its the highest that is 
    being started. Bed and breakfast INNS has also started being famous among people 
    As for 1980 till 2020, we can see a vast difference. Travelers accommodation 
    started for only a few years. Before 2000 it stopped being open.'
  })
